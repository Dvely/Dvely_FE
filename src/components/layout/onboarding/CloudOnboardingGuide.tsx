 ...
 * 색·타이포로 옮겼다.
 *
 * 주의: 비용 숫자와 보안 문구는 백엔드 C2(EC2 배포) 설계 확정값이다. 그 기능이 실제로
 * 열릴 때 구현과 어긋나면 사용자에게 잘못된 약속이 된다 — 문구를 고칠 일이 생기면
 * 여기와 백엔드 설계를 함께 봐야 한다.
 */
...
const STEPS = [
  ...
  },
  {
    ...
    body: 'Qeploy의 클라우드 연결 화면에서 안내를 따라가면 됩니다. 화면이 알려주는 대로 권한을 한 번 허용하고, 나온 값 하나를 붙여넣으면 연결이 끝납니다.',
    points: [
      'IAM이 무엇인지 몰라도 됩니다 — 필요한 권한은 Qeploy가 최소한으로만, 자동으로 준비합니다',
      '막히는 지점이 있으면 그 화면에 "다음에 뭘 하면 되는지"가 항상 표시됩니다',
    ],
    tags: ['약 5분', '한 번만'],
  },
  {
    ...
    points: [
      ...
      '더 이상 필요 없으면 서버 종료로 언제든 멈추고 청구를 끊을 수 있습니다',
    ],
    tags: ['몇 분', '매번 승인'],
  },
];
...
const SAFETY_POINTS = [
  'Qeploy는 이 배포에 필요한 것 외엔 당신 계정의 다른 자원을 만지지 못합니다. 권한을 딱 그만큼만 받습니다.',
