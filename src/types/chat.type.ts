...
const postConversationMessageCreateReqSchema = z.object({
  ...
  /**
   * 이 메시지로 도는 에이전트가 쓸 AI 제공자. **필수다.**
   *
   * 생략하면 서버가 자기 기본 제공자로 실행하고, 그 실행은 운영자 키로 과금된다.
   * 공개 서비스에서 열어 둘 수 없는 경로라 스키마에서 막는다 — 화면 어느 자리에서
   * 부르든 빈 값이 나갈 수 없다. BE #364 이후 서버도 생략을 400 으로 거절하므로
   * 여기서 먼저 걸러 주는 편이 실패 지점이 가깝다.
   *
