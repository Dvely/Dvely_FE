...
const postProjectCreateReqSchema = z.object({
  ...
  startMode: startModeSchema,
  /**
   * 사용자가 고른 콘텐츠 템플릿. `startMode: 'template'` 과 **같이** 보내야 한다 —
   * `blank` 로 보내면 서버가 이 값을 조용히 null 로 버린다(오류는 안 난다).
   *
   * 반드시 템플릿 카탈로그(`GET /templates`)의 `templateId` 여야 한다. 모르는 값은 400 이고,
   * 표시 이름(`학원 홈`)도 400 이다 — 서버가 `[a-z0-9][a-z0-9_-]{0,49}` 만 받는다.
   *
   * 여기서는 저장만 된다. 실제로 템플릿이 깔리는 것은 첫 CODE 스텝에서 컨테이너가 비어
   * 있을 때이고, 그래서 **만들 때 한 번만** 정할 수 있다 — 나중에 얹을 길이 없다.
   *
